...
import heaterSetup_mtk, homing_mtk, setpoint_wp, motorSetup_mtk
import modbus_tk
...
class CookProfile():
    ...
    #csv file defined as COM_PORT,platenTime,gapTime,cookTime,cookZone,hoodHeight,KeyControl,file_name
    platenTime = 10
    gapTime = 10
    cookTime = 152
    cookZone = 3050
    ...
    def setProfile(self, arg):
        ...
        self.gapTime = int(arg[2])
        self.cookTime = int(arg[3])
        self.cookZone = int(arg[4])
        self.hoodHeight = int(arg[5])
        self.KeyControl = arg[6]
        self.file_name = strftime(arg[7], time.localtime())
...
def main():
    ...
    while True:
        ...
        with open(openName, 'w') as log:
            ...
            fieldnames = (
                'Time', 'RearTemp', 'MiddleTemp', 'FrontTemp', 'UpGrate', 'LoGrate', 'RearPWM', 'UpPWM', 'MiddlePWM','LoPWM',
                'FrontPWM')
            ...
            while timeCal(startTime) < cooker.totalTime:
                if currentTime != int(timeCal(startTime)):
                    ...
                    if done:
                        break
                if timeCal(startTime) > CookProfile.gapTime + CookProfile.platenTime and gflag == 0:
                    gflag = 1
